# 智能问答工具 System Prompt 集成文档

## 概述

成功为智能问答工具（knowledge_qa）添加了专用的system prompt，指导Agent在调用该工具时如何更好地理解和响应用户查询。
...
```
基于图片的智能知识问答工具
使用多模态模型（qwen3-vl-plus）对图片进行深度理解和分析
结合用户问题提供准确、有用的回答
```
...
3. **调用流程**
   - 必须先通过检索工具获取图片ID
   - 然后调用knowledge_qa工具
   - 使用image_uuid、question和context参数
...
5. **不同场景的响应规范**
   - 植物问题：提供识别、养护说明和实用建议
   - 情感分析：描述情绪状态并生成适当的祝福或消息
   - 物体识别：列出识别的项目并提供相关推荐（菜谱、使用技巧等）
...
7. **错误处理**
   - 当图片内容不足以回答问题时，诚实承认局限性

### 3. ✅ 集成到agent_service.py

**修改位置**：`app/services/agent_service.py` 第197-213行
...
**更新工具列表文档**：
- 在`_register_core_tools()`方法的文档注释中添加了knowledge_qa工具
...
- ✅ Agent初始化和工具注册 - 通过
- ✅ 工具描述完整性 - 通过
- ✅ 已注册12个工具，包括knowledge_qa
- ✅ 工具包含所有必要参数（image_uuid, question, context）
...
- 使用RestfulApi定义工具
- 通过add_tools注册到Agent
- 在prompt_template中添加使用指导

## 后续优化建议

1. **多轮对话**：支持上下文记忆，实现连续问答
2. **多图片分析**：支持同时分析多张图片
3. **知识库集成**：接入外部知识库，增强回答准确性
4. **用户反馈**：收集用户对回答质量的反馈，持续优化
5. **性能监控**：记录工具调用次数和响应时间，优化性能

## 总结

✅ **已完成**：
- 分析现有prompt template结构
- 设计智能问答工具的system prompt
- 集成到agent_service.py
- 验证代码逻辑正确性
- 测试智能问答功能
