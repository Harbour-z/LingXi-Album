...
- Zustand 5.0.10 ：轻量级状态管理库，用于管理全局状态
  - chatStore ：管理sessionId、messages、isLoading、error等状态
  - unifiedStore ：管理搜索状态（searchQuery、topK、searchResults、lastSearchQuery）
  - conversationStore ：管理对话历史，支持IndexedDB持久化存储
  - voiceStore ：管理语音识别状态（连接、录音、转录等）
...
├── store/            # 状态管理
│   ├── chatStore.ts         # 聊天状态管理
│   ├── unifiedStore.ts      # 统一状态管理（搜索、topK、搜索结果）
│   ├── conversationStore.ts # 对话历史状态管理（IndexedDB持久化）
│   ├── themeStore.ts         # 主题状态管理
│   └── voiceStore.ts        # 语音识别状态管理
├── hooks/            # 自定义Hooks
│   ├── useDebounce.ts                # 防抖Hook
│   ├── usePerformanceMonitor.ts       # 性能监控Hook
│   ├── useUnifiedDataFlow.ts         # 统一数据流Hook
│   └── useUnifiedDataPersistence.ts  # 数据持久化Hook
├── services/         # 服务层
│   └── conversationStorage.ts  # IndexedDB对话历史存储服务
├── types/            # 类型定义
...
### 3.6 实时语音识别与对话历史管理
创新描述 系统集成了实时语音识别和对话历史持久化功能，大幅提升了用户交互的便捷性和数据的可追溯性。

技术实现

- WebSocket实时通信：通过WebSocket与后端ASR服务建立实时双向通信，支持音频流式传输
- 语音波形可视化：前端AudioWaveform组件实时显示语音波形，提供直观的音频反馈
- VAD语音检测：服务端自动检测语音段落，400ms静音自动结束识别
- IndexedDB持久化：使用IndexedDB存储对话历史，支持离线访问和跨会话保留
- 对话历史管理：conversationStore管理对话列表，支持创建、加载、更新、删除等操作
- 状态管理：voiceStore统一管理语音识别的连接、录音、转录等状态

技术优势

- 高准确率：支持中英文双语，识别准确率95%以上
- 低延迟：实时流式传输，识别延迟低于1秒
- 离线支持：IndexedDB支持离线数据访问
- 数据安全：本地存储，数据不上传云端
- 可扩展性：支持多种语言和方言
...
│  │  │                       MainLayout                                     │    │  │
│  │  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐              │    │  │
│  │  │  │  ChatPage    │  │ GalleryPage  │  │ UploadPage   │  Architecture │    │  │
│  │  │  │  (智能对话)   │  │  (图片画廊)   │  │  (图片上传)   │    Page       │    │  │
│  │  │  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘              │    │  │
│  │  │         │                 │                 │                       │    │  │
│  │  │  ┌──────▼─────────────────▼─────────────────▼───────┐               │    │  │
│  │  │  │              共享组件                            │               │    │  │
│  │  │  │  SearchBox, ImageCard, MessageList, ImageGrid    │               │    │  │
│  │  │  └──────┬───────────────────────────────────────────┘               │    │  │
│  │  └─────────┼───────────────────────────────────────────────────────────┘    │  │
│  └────────────┼───────────────────────────────────────────────────────────────┘  │
│                │                                                                    │
...
│  │  │  - sendMessage (120s超时, AbortError处理)                           │  │  │
│  │  └──────────────────────────────────────────────────────────────────────┘  │  │
│  └─────────────┬───────────────────────────────────────────────────────────────┘  │
│                │                                                                    │
│  ┌─────────────▼───────────────────────────────────────────────────────────────┐  │
│  │                       API 客户端层 (API Client)                               │  │
│  │  ┌──────────────────────────────────────────────────────────────────────┐  │  │
│  │  │  Axios Instance (baseURL='/api/v1', timeout=600000ms)                 │  │  │
│  │  │  - agent.ts: sendChatMessage, createSession, deleteImagesByRecommendation│  │
│  │  │  - search.ts: searchByText, searchByImage, searchHybrid, searchMeta  │  │  │
│  │  │  - storage.ts: uploadImage, getImage, deleteImage, listImages       │  │  │
│  │  │  - social.ts: generateCaption                                       │  │  │
│  │  │  - image_edit.ts: editImage                                        │  │  │
│  │  └──────────────────────────────────────────────────────────────────────┘  │  │
│  └─────────────┬───────────────────────────────────────────────────────────────┘  │
└────────────────┼──────────────────────────────────────────────────────────────────┘
...
│  │  │                          路由层 (Routers)                               │  │  │
│  │  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                │  │  │
│  │  │  │  agent.py    │  │  search.py   │  │  storage.py  │  ...            │  │  │
│  │  │  │  (901行)     │  │  (399行)     │  │  (486行)     │                │  │  │
│  │  │  │  /agent/chat │  │  /search/*   │  │  /storage/*  │                │  │  │
│  │  │  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘                │  │  │
│  │  │         │                 │                 │                         │  │  │
│  │  └─────────┼─────────────────┼─────────────────┼─────────────────────────┘  │  │
│  └─────────────┼─────────────────┼─────────────────┼────────────────────────────┘  │
...
│  │  ┌──────────────────────────────────────────────────────────────────────┐  │  │
│  │  │  EmbeddingService (单例)                                             │  │  │
│  │  │  - Qwen3-VL-Embedding-2B (2560维)                                    │  │  │
│  │  │  - 支持本地推理 + 阿里云DashScope API                                  │  │  │
│  │  │  - 指令: "Represent this text/image for retrieval."                  │  │  │
│  │  └──────────────────────────────────────────────────────────────────────┘  │  │
│  │  ┌──────────────────────────────────────────────────────────────────────┐  │  │
│  │  │  VectorDBService (单例)                                              │  │  │
│  │  │  - Qdrant客户端 (本地/Docker/云)                                      │  │  │
│  │  │  - COSINE距离, query_points()方法                                     │  │  │
│  │  │  - payload索引: tags(关键词), created_at(日期)                         │  │  │
│  │  └──────────────────────────────────────────────────────────────────────┘  │  │
│  │  ┌──────────────────────────────────────────────────────────────────────┐  │  │
│  │  │  SearchService (单例)                                                 │  │  │
│  │  │  - search_by_text: 语义搜索                                           │  │  │
│  │  │  - search_by_date_text: 日期解析 (1.18, 1月18日, 2026-01-18)          │  │  │
...
│  │  ┌──────────────────────────────────────────────────────────────────────┐  │  │
│  │  │  AgentService (单例)                                                  │  │  │
│  │  │  - Openjiuwen ReActAgent集成                                          │  │  │
│  │  │  - 9个工具: semantic_search, search_by_image, meta_search, etc.       │  │  │
...
│  │  │  5. agent_execute_action        (执行操作: search/upload/delete/update)│  │  │
...
│  │  │  - Vector: 2560维 (Qwen3-VL-Embedding-2B)                             │  │  │
│  │  │  - Distance: COSINE                                                   │  │  │
...
└───────────────────────────────────────────────────────────────────────────────────────┘
                             │
┌────────────────────────────▼────────────────────────────────────────────────────────┐
│                    基础设施/部署层 (Infrastructure/Deployment Layer)                    │
│                                                                                       │
│  ┌───────────────────────────────────────────────────────────────────────────────┐  │
│  │                        配置管理 (config.py)                                     │  │
│  │  - Pydantic Settings                                                          │  │
...
│  │  - ChatRequest/Response, ImageRecommendation, ImageEditRequest/Response        │  │
│  └───────────────────────────────────────────────────────────────────────────────┘  │
│  ┌───────────────────────────────────────────────────────────────────────────────┐  │
│  │                        外部服务                                                │  │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                      │  │
│  │  │  阿里云       │  │  OpenAI兼容   │  │  Qdrant云    │                      │  │
│  │  │  DashScope   │  │  API服务     │  │  服务        │                      │  │
│  │  │  (Embedding) │  │  (LLM/Agent) │  │  (可选)      │                      │  │
│  │  └──────────────┘  └──────────────┘  └──────────────┘                      │  │
│  └───────────────────────────────────────────────────────────────────────────────┘  │
│  ┌───────────────────────────────────────────────────────────────────────────────┐  │
│  │                        部署模式                                                │  │
...
**报告类型**：技术架构评估与风险审计
**评估日期**：2026-03-08
**评估范围**：安全性、性能、代码质量、可扩展性、技术债务

## 一、执行摘要

本评估报告基于对项目代码库的深度分析，识别出**18个关键问题**，包括3个严重（P0）、8个重要（P1）、7个一般（P2）级别的问题。
...
**优先改进建议**：
1. CORS配置安全加固（P0，1小时工作量）
2. 添加API限流机制（P0，4小时工作量）
3. Agent超时控制（P0，2小时工作量）
...
#### SEC-001: CORS配置允许所有来源

**位置**：`app/main.py:193-199`
**风险等级**：🔴 严重 (CVSS 8.5)
**修复方案**：修改为白名单模式
...
|------|---------|-------|------|
| 首次搜索延迟 | ~10s | <3s | 70%↓ |
| Agent响应时间 | 2-30s | <5s | 不稳定 |
| 向量检索时间 | <100ms | <50ms | 50%↓ |
...
|------|------|---------|
| 模型冷启动 | 无预热机制 | 启动预加载 |
| Agent API | 无超时控制 | 添加超时 |
| 同步I/O | 阻塞事件循环 | 使用aiofiles |
...
| ID | 描述 | 优先级 | 工作量 |
|----|------|--------|--------|
| TD-001 | OpenJiuwen固定版本 | P0 | 16h |
| TD-002 | 缺少单元测试 | P1 | 40h |
| TD-003 | 错误处理不统一 | P1 | 12h |
