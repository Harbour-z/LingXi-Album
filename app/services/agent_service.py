...
class AgentService:
    ...
    def __init__(self):
        ...
        self._current_conversation_id: ContextVar[Optional[str]] = ContextVar(
    ...
    def initialize(
        ...
        **kwargs
        ...
        if settings.AGENT_ENABLED and settings.OPENAI_API_KEY:
            ...
            try:
                logger.info(
                    f"正在配置ReAct Agent (Provider: {settings.AGENT_PROVIDER}, Model: {settings.OPENAI_MODEL_NAME})...")
    ...
    def _setup_agent(self, settings):
        ...
        if ssl_verify and not ssl_cert:
            logger.warning(
                "LLM_SSL_VERIFY=true but LLM_SSL_CERT is missing; forcing LLM_SSL_VERIFY=false")
            ssl_verify = False
    ...
    def _register_core_tools(self):
        ...
        tool_semantic_search_images = RestfulApi(
            ...
            params=[
                Param(name="query", description="用户的搜索描述，例如 '穿红衣服的人' 或 '海边的日落'",
                      param_type="string", required=True),
                Param(name="instruction", description="自定义指令（可选）", param_type="string",
                      required=False, default_value="", method="Query"),
                Param(name="top_k", description="返回结果的数量，默认为10", param_type="integer",
                      default_value=10, required=False, method="Query"),
                Param(name="score_threshold", description="相似度阈值（可选），范围0-1",
                      param_type="number", required=False, method="Query"),
                Param(name="tags", description="标签过滤（可选），逗号分隔的字符串，例如 'cat,outdoor'",
                      param_type="string", required=False, default_value="", method="Query")
            ],
        ...
        tool_search_by_image_id = RestfulApi(
            ...
            params=[
                Param(name="image_id", description="查询图片的ID",
                      param_type="string", required=True),
                Param(name="instruction", description="自定义指令（可选）", param_type="string",
                      required=False, default_value="", method="Query"),
                Param(name="top_k", description="返回结果的数量，默认为10", param_type="integer",
                ...
                Param(name="score_threshold", description="相似度阈值（可选），范围0-1",
                      param_type="number", required=False, method="Query"),
                Param(name="tags", description="标签过滤（可选），逗号分隔的字符串",
                      param_type="string", required=False, default_value="", method="Query")
            ],
        ...
        tool_meta_search_images = RestfulApi(
            ...
            params=[
                Param(name="date_text", description="日期文本，支持格式：1.18 / 1月18日 / 2026-01-18",
                      param_type="string", required=False, method="Query"),
                Param(name="tags", description="标签过滤（可选），逗号分隔的字符串，例如 'cat,outdoor'",
                      param_type="string", required=False, method="Query"),
                Param(name="top_k", description="返回结果数量，默认为5", param_type="integer",
                      default_value=5, required=False, method="Query")
            ],
        ...
        tool_meta_search_hybrid = RestfulApi(
            ...
            params=[
                ...
                Param(name="tags", description="标签过滤（可选），逗号分隔的字符串，例如 'cat,outdoor'",
                      param_type="string", required=False, method="Query"),
                Param(name="query", description="语义查询描述，例如 '海边日落'",
                      param_type="string", required=False, method="Query"),
                Param(name="top_k", description="返回结果数量，默认为5", param_type="integer",
                      default_value=5, required=False, method="Query")
            ],
        ...
        tool_agent_execute = RestfulApi(
            ...
            params=[
                Param(name="action", description="动作类型：search/delete/update/analyze",
                      param_type="string", required=True, method="Body"),
                Param(name="parameters", description="动作参数对象，包含查询、图片ID等信息", param_type="object", required=True, method="Body", schema=[
                    {"name": "query", "description": "搜索查询文本",
                        "type": "string", "required": False},
                    {"name": "image_id", "description": "图片ID",
                        "type": "string", "required": False},
                    {"name": "top_k", "description": "返回结果数量",
                        "type": "integer", "required": False}
                ]),
                Param(name="context", description="上下文信息（可选）", param_type="object", required=False, method="Body", schema=[
                    {"name": "session_id", "description": "会话ID",
                        "type": "string", "required": False},
            ...
            response=[
                ...
                Param(name="data", description="执行结果", param_type="object", schema=[
                    {"name": "results", "description": "搜索结果列表",
                        "type": "array", "required": False},
                    {"name": "total", "description": "结果总数",
                        "type": "integer", "required": False}
                ])
            ]
        )
        ...
        tool_generate_social_media_caption = RestfulApi(
            ...
            params=[
                Param(name="image_uuid", description="存储在系统storage中的图片的唯一标识符（UUID）。通常来自先前的图片检索结果。",
                      param_type="string", required=True),
                Param(name="style", description="文案风格，例如：'幽默'、'文艺'、'简洁'、'详细'。",
                      param_type="string", required=False, default_value="简洁"),
                Param(name="purpose", description="文案用途，例如：'产品推广'、'生活分享'、'活动宣传'。",
                      param_type="string", required=False, default_value="生活分享")
            ],
        ...
        tool_recommend_images = RestfulApi(
            ...
            params=[
                Param(
                    name="images", description="图片ID列表（最多10张），JSON数组格式：[\"id1\", \"id2\"]", param_type="array", required=True),
                Param(name="user_preference", description="用户偏好或分析维度（可选），例如：'我更喜欢构图好的'、'关注色彩搭配'",
                      param_type="string", required=False, default_value="")
            ],
        ...
        tool_edit_image = RestfulApi(
            ...
            params=[
                Param(name="image_id", description="要编辑的图片ID（必须先通过检索工具获取图片ID）",
                      param_type="string", required=True, method="Body"),
                Param(name="prompt", description="编辑提示词，例如：'将图片转换为动漫风格'",
                      param_type="string", required=True, method="Body"),
                Param(name="negative_prompt", description="反向提示词，描述不希望出现的内容，例如：'模糊，多余的手指'",
                      param_type="string", required=False, default_value=" ", method="Body"),
                Param(name="prompt_extend", description="是否开启智能提示词改写功能（默认为 true，会优化简单描述的生成效果）",
                      param_type="boolean", required=False, default_value=True, method="Body"),
                Param(name="n", description="生成图片数量（1-6张）", param_type="integer",
                      required=False, default_value=1, method="Body"),
                Param(name="size", description="输出图片分辨率，格式为 '宽*高'，例如 '1024*1536'",
                      param_type="string", required=False, method="Body"),
                Param(name="watermark", description="是否添加水印", param_type="boolean",
                      required=False, default_value=False, method="Body"),
                Param(name="seed", description="随机数种子（可选）",
                      param_type="integer", required=False, method="Body"),
                Param(name="style_tag", description="风格标签，用于元数据记录，例如：'anime', 'cartoon'",
                      param_type="string", required=False, method="Body")
            ],
            ...
            response=[
                ...
                Param(name="data", description="编辑结果，包含生成的图片信息", param_type="object", schema=[
                    Param(name="success", description="操作是否成功",
                          param_type="boolean", required=True),
                    Param(name="saved_images", description="保存的图片列表", param_type="array", required=True, schema=[
                        Param(name="image_id", description="图片ID",
                              param_type="string", required=True),
                        Param(name="url", description="图片URL",
                              param_type="string", required=True),
                        Param(name="metadata", description="元数据信息", param_type="object", required=True, schema=[
                            Param(name="source_image_id", description="源图片ID",
                                  param_type="string", required=False),
                            Param(name="edit_prompt", description="编辑提示词",
                                  param_type="string", required=False),
                            Param(name="edit_style", description="编辑风格",
                                  param_type="string", required=False),
                            Param(name="edit_model", description="使用的编辑模型",
                                  param_type="string", required=False),
                            Param(name="edit_parameters", description="编辑参数", param_type="object", required=False, schema=[
                                Param(name="negative_prompt", description="反向提示词",
                                      param_type="string", required=False),
                                Param(name="prompt_extend", description="是否开启智能提示词改写",
                                      param_type="boolean", required=False),
                                Param(name="n", description="生成图片数量",
                                      param_type="integer", required=False),
                                Param(name="size", description="输出图片分辨率",
                                      param_type="string", required=False),
                                Param(name="watermark", description="是否添加水印",
                                      param_type="boolean", required=False),
                                Param(name="seed", description="随机数种子",
                                      param_type="integer", required=False)
                            ]),
                            Param(name="edit_time", description="编辑时间",
                                  param_type="string", required=False),
                            Param(name="tags", description="标签列表", param_type="array", required=False, schema=[
                                Param(name="tag", description="标签名称",
                                      param_type="string", required=False)
                            ])
                        ])
                    ]),
                    Param(name="total_generated", description="生成的图片总数",
                          param_type="integer", required=True),
                    Param(name="total_saved", description="成功保存的图片数量",
                          param_type="integer", required=True)
                ])
            ]
        )
        ...
        tool_generate_pointcloud = RestfulApi(
            ...
            params=[
                Param(name="image_id", description="来源图片ID（必须先通过检索工具获取）",
                      param_type="string", required=True, method="Body"),
                Param(name="quality", description="生成质量：'balanced'（高质量）或 'fast'（快速模式）",
                      param_type="string", required=False, default_value="balanced", method="Body"),
                Param(name="async_mode", description="是否异步生成（推荐true，立即返回任务ID）",
                      param_type="boolean", required=False, default_value=True, method="Body")
            ],
            ...
            response=[
                ...
                Param(name="data", description="点云生成结果", param_type="object", schema=[
                    Param(name="pointcloud_id",
                          description="点云ID", param_type="string"),
                    Param(
                        name="status", description="生成状态（pending/processing/completed/failed）", param_type="string"),
                    Param(name="source_image_id",
                          description="来源图片ID", param_type="string"),
                    Param(name="file_path", description="PLY文件路径",
                          param_type="string"),
                    Param(name="download_url", description="下载URL",
                          param_type="string"),
                    Param(name="view_url", description="3D预览URL（浏览器可直接查看）",
                          param_type="string"),
                    Param(name="created_at", description="创建时间",
                          param_type="string"),
                    Param(name="completed_at", description="完成时间",
                          param_type="string"),
                    Param(name="file_size", description="文件大小(bytes)",
                          param_type="integer"),
                    Param(name="point_count", description="点云点数",
                          param_type="integer"),
                    Param(name="error_message",
                          description="错误信息", param_type="string")
                ])
            ]
        )
        ...
        tool_knowledge_qa = RestfulApi(
            ...
            params=[
                Param(name="image_uuid", description="存储在系统storage中的图片的唯一标识符（UUID）。通常来自先前的图片检索结果。",
                      param_type="string", required=True),
                Param(name="question", description="用户的问题，例如：'这是什么植物？'、'妈妈开心吗？'、'有什么食材？'",
                      param_type="string", required=True),
                Param(name="context", description="额外上下文信息（可选），例如：'这是妈妈60岁生日'、'准备做晚餐'",
                      param_type="string", required=False, default_value="")
            ],
        ...
        tool_get_pointcloud_status = RestfulApi(
            ...
            description="查询点云任务状态和预览链接。当用户要求查看历史点云任务、查询任务进度、获取预览链接或下载链接时使用此工具。例如：'查看历史任务的链接'、'查询点云任务状态'、'查看预览链接'。此工具不会创建新任务，只查询现有任务状态。",
            params=[
                Param(name="pointcloud_id", description="点云任务ID（从之前的任务创建或历史记录中获取）",
                      param_type="string", required=True, method="Body")
            ],
            path=f"{api_base}{api_prefix}/pointcloud/status",
            headers={"Content-Type": "application/json"},
            method="POST",
            response=[
                Param(name="status", description="响应状态", param_type="string"),
                Param(name="message", description="响应消息", param_type="string"),
                Param(name="data", description="点云任务信息", param_type="object", schema=[
                    Param(name="pointcloud_id",
                          description="点云ID", param_type="string"),
                    Param(
                        name="status", description="生成状态（pending/processing/completed/failed）", param_type="string"),
        ...
        self._tools.append(tool_get_pointcloud_status)
    ...
    async def chat(self, query: str, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        logger.info(
            f"[Agent] 开始处理查询: '{query}', session_id: {session_id}, conversation_id: {conversation_id}")
        ...
        try:
            ...
            if not is_pointcloud:
                ...
                if (len(images) > 0 and
                    ...
                    if len(images) > 1:
                        ...
                        recommendation["recommended_image_id"] = images[0].get(
                            "id")
                        recommendation["alternative_image_ids"] = [
                            img.get("id") for img in images[1:]]
            ...
            if self._detect_pointcloud_generation(query):
                pointcloud_id = self._extract_pointcloud_id_from_response(
                    response)
                if pointcloud_id:
            ...
            logger.error(
                f"[Agent] 异常详情 - query: {query}, session_id: {session_id}")
            return {
    ...
    async def _monitor_pointcloud_generation(
        ...
        logger.info(
            f"[Agent] 开始监控点云生成任务: {pointcloud_id}, 最大等待时间: {max_wait_seconds}秒")
        ...
        if initial_pointcloud_info:
            ...
            logger.info(
                f"[Agent] 初始状态检查 - 状态: {initial_status}, view_url: {initial_view_url}, 点数: {initial_point_count}")
        ...
        while elapsed_time < max_wait_seconds:
            try:
                ...
                logger.info(
                    f"[Agent] 轮询点云状态: {status}, view_url={view_url}, file_path={file_path}, point_count={point_count}, 已等待: {elapsed_time}秒")

                # 只要状态是completed就认为生成成功，不一定需要view_url
                if status == "completed":
                    logger.info(
                        f"[Agent] ✓ 点云生成完成! ID: {pointcloud_id}, 点数: {point_count}, 预览URL: {view_url}")
        ...
        logger.warning(
            f"[Agent] ⏰ 点云生成超时（{max_wait_seconds}秒），任务ID: {pointcloud_id}")
        return None
    ...
    async def _monitor_and_update_pointcloud(
        ...
        try:
            ...
            logger.info(
                f"[Agent] _monitor_pointcloud_generation 返回: {view_url}")
                ...
                if pointcloud_info:
                    ...
                    if session_id:
                        ...
                        if session:
                            ...
                            logger.info(
                                f"[Agent] ✅ 会话已更新，点云完成事件已添加: {session_id}")
                        else:
                ...
                if is_pointcloud_generation:
                    ...
                    try:
                        ...
                        if storage_svc.is_initialized:
                            ...
                            if not image_info:
                                logger.info(
                                    f"[Agent] 在点云生成场景中，排除非图片ID: {image_id}")
                                continue
                    except Exception as e:
    ...
    def _extract_recommendation_from_response(
        ...
        logger.info(
            f"[Agent] 从回复中提取到 {len(all_image_ids)} 个图片ID: {all_image_ids}")
        ...
        if not all_image_ids and context_images:
            all_image_ids = [img.get("id")
                             for img in context_images if img.get("id")]
        ...
        if all_image_ids:
            ...
            if recommended_id:
                ...
                recommendation["user_prompt_for_deletion"] = len(
                    recommendation["alternative_image_ids"]) > 0
            else:
                # 没有明确的推荐，使用第一个ID作为推荐
